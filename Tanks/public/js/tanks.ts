...
export class Game{
  ...
  socket;
  localTank;
  tanks = []; //Tanks (other than the local tank)
  balls = [];
  $arena;

  constructor(arenaId, w: number, h: number, socket) {
    this.width = w;
    this.height = h;
    this.$arena = $(arenaId);
  ...
  killTank(tank) {
    ...
    //place explosion
    this.$arena.append('<img id="expl' + tank.id + '" class="explosion" src="/static_assets/img/explosion.gif">');
...
class Tank{
  id; type; speed; w; h; baseAngle; cannonAngle; x; y; dir; game; isLocal; hp; dead;
  $arena; $body; $cannon; $info;
